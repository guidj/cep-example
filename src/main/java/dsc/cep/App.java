...
public class App {
	public static void main(String[] args) {
		...
		EPStatement highTempStatement = cepAdm
				.createEPL("SELECT istream temperature FROM TemperatureEventStream.win:time(30 sec) "
		...
		averageTempStatement.setSubscriber(new AverageSubscriber(
				"Average Temperature"));

		// pass cepTR to event listener to send new events
		// create event to receive average

		SimpleTemperatureEventGenerator gen = new SimpleTemperatureEventGenerator(
				cepRT, 1000, 50);
