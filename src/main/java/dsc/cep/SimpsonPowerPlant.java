...
public class SimpsonPowerPlant {
	...
	public static void main(String[] args) {
		...
		EPStatement temperatureStatement = cepAdm
				.createEPL("SELECT istream sensor, temperature FROM TemperatureEventStream.win:time(10 sec) "
		...
		EPStatement warningStatement = cepAdm
				.createEPL("SELECT istream * FROM TemperatureEventStream "
						+ "MATCH_RECOGNIZE( " + "PARTITION BY sensor "
						+ "MEASURES A as tempA, B as tempB " + "PATTERN (A B) "
						+ "DEFINE " + "	A as A.temperature > " + THRESHOLD_TEMP
		...
		PowerPlantTemperatureGenerator gen = new PowerPlantTemperatureGenerator(
				cepRT, 1000, 500);
