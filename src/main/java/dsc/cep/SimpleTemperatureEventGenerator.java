...
public class SimpleTemperatureEventGenerator {
	...
	public SimpleTemperatureEventGenerator(EPRuntime cepRT, int sleepTime,
			int neo) {
	...
	public void startSendingTemperatureReadings() {
		...
		while (count < this.noe) {
			TemperatureSensorEvent event = new TemperatureSensorEvent(
					"sensorX", new Random().nextInt(150), new Date());
			// System.out.println("Sending " + event.getTemperature() + " at " +
			// event.getTimeStamp());
